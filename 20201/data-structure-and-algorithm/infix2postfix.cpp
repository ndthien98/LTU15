...
void calculatePrefixValue();

void inputExpression()
{
  cout << "Input infix expression: ";
  char EXP_REGEX[] = "^[-+(]*[[:digit:]]+[)]*([-+*/][-+(]*[[:digit:]]+[)]*)*$";
  cin >> expression;
  cout << expression;
  if (!regex_match(expression, regex(EXP_REGEX)))
  {
...
int main()
{
  ...
  // main flow: input -> check valid expression -> convert to prefix -> calculate value
...
int postfix(char *expression)
{
  ...
  for (i = 0, j = -1; expression[i]; ++i)
  {
    ...
    {
      while (!stackEmpty(stack) && precedence(expression[i]) <=
