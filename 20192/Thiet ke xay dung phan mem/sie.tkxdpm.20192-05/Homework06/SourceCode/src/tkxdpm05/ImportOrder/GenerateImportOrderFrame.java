...
public class GenerateImportOrderFrame extends javax.swing.JFrame {
    ...
    private void btnGenerateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGenerateActionPerformed
        ...
        if (listSaleOrderDTOs.get(index).getStatus().equals("GENERATED")) {
            ...
            generateImportOrder(listSaleOrderDTOs.get(index));
    ...
    private void generateImportOrder(SaleOrderDTO saleOrderDTO) {
        listImportOrderDTOs = new ArrayList<>();

        listSaleOrderDetailDTOs = ImportOrderController.getInstance().readAllSaleOrderDetail(saleOrderDTO);

        listProvideDTOs = ImportOrderController.getInstance().readAllProvide();
        
        for (SaleOrderDetailDTO detail : listSaleOrderDetailDTOs) {
            ImportOrderDTO importOrderDTO = new ImportOrderDTO();

            int quantity = detail.getQuantity();
            int merchandiseId = detail.getMerchandiseId();
            int saleOrderId = detail.getSaleOrderId();

            for (ProvideDTO provide : listProvideDTOs) {
                if (provide.getMerchandiseId() == merchandiseId
                        && provide.getInstock() >= quantity) {

                    importOrderDTO.setImportOrderId(-1);

                    importOrderDTO.setMerchandiseId(merchandiseId);
                    importOrderDTO.setQuantity(quantity);
                    importOrderDTO.setSaleOrderId(saleOrderId);

                    importOrderDTO.setSiteId(provide.getSiteId());
                    importOrderDTO.setStatus("NEW");
                    importOrderDTO.setDeliveryMean("2020-12-12");
